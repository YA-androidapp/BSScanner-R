...
setML <- function(consolidated,stitle,n,x1,x2,isY){
  ...
  if(consolidated>1){
    ...
    if(isY==0){
      tryCatch(
        {
          t<-tuneRF(d.train[,c(-1,-2)],d.train[,2],doBest=T,ntree=500)
          cat("mtry:",t$mtry,"\n")
          v.result <<- randomForest(v$x1 ~ v$dx + v$dy + v$BSSID1.1 + v$BSSID2.1 + v$BSSID3.1 + v$BSSID1.2 + v$BSSID2.2 + v$BSSID3.2, data=d.train, na.action="na.omit",mtry=t$mtry,ntree=500)
        }, 
        error = function(e) {
          cat("E!\n")
      ...
      tryCatch(
        {
          t<-tuneRF(d.train[,c(-1,-2)],d.train[,1],doBest=T,ntree=500)
          cat("mtry:",t$mtry,"\n")
          v.result <<- randomForest(v$y1 ~ v$dx + v$dy + v$BSSID1.1 + v$BSSID2.1 + v$BSSID3.1 + v$BSSID1.2 + v$BSSID2.2 + v$BSSID3.2, data=d.train, na.action="na.omit",mtry=t$mtry,ntree=500)
        }, 
        error = function(e) {
          cat("E!\n")
      ...
      tryCatch(
        {
          t<-tuneRF(d.train[,c(-1,-2)],d.train[,2],doBest=T,ntree=500)
          cat("mtry:",t$mtry,,"\n")
          v.result <<- randomForest(v$x1 ~ v$BSSID1.1 + v$BSSID2.1 + v$BSSID3.1, data=d.train, na.action="na.omit",mtry=t$mtry,ntree=500)
        }, 
        error = function(e) {
          cat("E!\n")
          v.result <<- randomForest(v$x1 ~ v$BSSID1.1 + v$BSSID2.1 + v$BSSID3.1, data=d.train, na.action="na.omit",mtry=3,ntree=500)
        },
        warning = function(e) {
          cat("W!\n")
          v.result <<- randomForest(v$x1 ~ v$BSSID1.1 + v$BSSID2.1 + v$BSSID3.1, data=d.train, na.action="na.omit",mtry=3,ntree=500)
      ...
      tryCatch(
        {
          t<-tuneRF(d.train[,c(-1,-2)],d.train[,1],doBest=T,ntree=500)
          cat("mtry:",t$mtry,"\n")
          v.result <<- randomForest(v$y1 ~ v$BSSID1.1 + v$BSSID2.1 + v$BSSID3.1, data=d.train, na.action="na.omit",mtry=t$mtry,ntree=500)
        }, 
        error = function(e) {
          cat("E!\n")
          v.result <<- randomForest(v$y1 ~ v$BSSID1.1 + v$BSSID2.1 + v$BSSID3.1, data=d.train, na.action="na.omit",mtry=3,ntree=500)
        },
        warning = function(e) {
          cat("W!\n")
          v.result <<- randomForest(v$y1 ~ v$BSSID1.1 + v$BSSID2.1 + v$BSSID3.1, data=d.train, na.action="na.omit",mtry=3,ntree=500)
...
main <- function(CONSOLIDATED,DFILTER,DFILTERVALUE,K,SIMIMIN,SIMIMAX,WIDE,ISTEST){
  stitle <- paste("RF_c",CONSOLIDATED,"d",DFILTER,"v",DFILTERVALUE,"k",K,"n",SIMIMIN,"x",SIMIMAX,"w",WIDE,"i",ISTEST,"t",0,sep="")
